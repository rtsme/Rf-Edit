...
# The client root's data is a mix of EUC-KR/cp949 text, CRLF text and binary
# blobs -- see docs/knowledge/client-file-formats.md. Unlike the server root,
# nothing here is known to be uniform ASCII/LF, so every byte is kept exactly
# as read: no text/eol conversion at all.
...
def find_client_verbatim(root):
    ...
    def keep(rel):
        ...
        if key in CLIENT_EXCLUDE_PATHS or key in CLIENT_ASSET_PATH_OVERRIDES:
            return False
        return os.path.splitext(key)[1].lower() not in CLIENT_ASSET_EXTS
